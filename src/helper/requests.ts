...
interface UpdateProductFromBasketRequestProps {
    data: null | {}
    error: null | Error
}

export function updateProductFromBasketRequest(basketId: string, productId: string, quantity: number): Promise<UpdateProductFromBasketRequestProps> {
    const url = makeUpdateProductFromBasketURL(basketId)
    console.log(`posted product to basket with URL: ${url}`)
    const respone = fetch(url, {
        method: 'POST',
        headers: {
